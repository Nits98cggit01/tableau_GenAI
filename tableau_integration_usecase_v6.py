...
def create_pdf(cg_logo_path,selected_workbook, selected_dashboard, lineage_path, selecteddashboard_folder,dashboardimgfile,narrationfolder):
    ...
    # Output the PDF
    output_path = os.path.join(narrationfolder,f'{selected_workbook}-{selected_dashboard}_narration.pdf')
    pdf.output(output_path)
    print(f'PDF created successfully at {output_path}')

def display_pdf(narration_folder,selected_workbook,selected_dashboard):
    filename = f'{selected_workbook}-{selected_dashboard}_narration.pdf'
    pdf_path = os.path.join(narration_folder,filename)  
...
def generate_csvs(metadata_file,portalname,template_folder):
    ...
    masterfile_path = os.path.join(template_folder,f'{portalname}_Master_metadata.csv')
    ...
    tableschemapath = os.path.join(template_folder,f'{portalname}_Table_schema.csv')
    ...
    datalinkpath = os.path.join(template_folder,f'{portalname}_MasterData_links.csv')
...
def get_prompt(project_path):
    # Read the CSV file
    try:
        df = pd.read_csv(project_path)
        df['Description'] = ""
        st.write(f'The df opened {project_path}')
...
def get_dashboard_link(portalname,server_endpoint_file,dashid,dashboard_df):
    with open(server_endpoint_file, 'r') as file:
...
def main():
    ...
    # lineagefile has been renamed to project_path
    project_path = os.path.join(ROOT_PATH, project_folder)

    metadata_folder = os.path.join(project_path,'metadata')

    if not os.path.exists(metadata_folder):
        os.makedirs(metadata_folder)
    
    wb_metadata_folder = os.path.join(metadata_folder,'workbook_metadata')
    dashb_metadata_folder = os.path.join(metadata_folder, 'dashboard_metadata')
    all_wb_metadata_folder = os.path.join(metadata_folder, 'site_metadata')

    if not os.path.exists(wb_metadata_folder):
        os.makedirs(wb_metadata_folder)

    if not os.path.exists(dashb_metadata_folder):
        os.makedirs(dashb_metadata_folder)

    if not os.path.exists(all_wb_metadata_folder):
        os.makedirs(all_wb_metadata_folder)
    

    # Placeholder for generating CSVs
    # generate_csvs(metadatafile, portalname, portalfolder)

    template_folder = os.path.join(project_path, 'template')
    narration_folder = os.path.join(project_path, 'narration')
    ...
    masterfile = os.path.join(template_folder, f'{portalname}_Master_metadata.csv')
    linkfile = os.path.join(template_folder, f'{portalname}_MasterData_links.csv')
    tableschemafile = os.path.join(template_folder, f'{portalname}_Table_schema.csv')
    # st.title("Report Matching Assistant")
    view_table_folder = os.path.join(project_path,'view table')
    ...
    if not st.session_state.authenticated:
        ...
        if st.button("Sign In"):
            ...
            server_endpoint_file = os.path.join(project_folder,f'serverendpoint.txt')
            with open(server_endpoint_file,'w') as file:
            ...
            if response.status_code == 200:
                ...
                if os.path.exists(viewsfile):
                    st.write(f'Views file for {portalname} already exists')
                else:
                    views_df = get_views(conn)
                    views_df.to_csv(viewsfile)

                if os.path.exists(workbookfile):
                    st.write(f'Workbook file for {portalname} already exists')
                else:
                    wb_df = get_workbook(conn)
                    wb_df.to_csv(workbookfile)
            
                if os.path.exists(dashboardfile):
                    st.write(f'The Dashboard file for {portalname} already exists')
                else:
                    views_df = pd.read_csv(viewsfile)
                    get_dashboard_details = get_dashboard(views_df)
                    get_dashboard_details.to_csv(dashboardfile)

                if os.path.exists(viewsfile) and os.path.exists(dashboardfile) and os.path.exists(workbookfile):
                    st.success(f'All required files exists, proceeding with the service page')
                    ...
                    st.warning(f'Required files does not exist')
        ...
        if st.session_state.service_selected == 'Dashboard metadata extraction' and st.session_state.authenticated:
            ...
            tabs = st.sidebar.radio('Metadata', ['Metadata by workbook', 'Metadata by dashboard', 'All workbooks metadata'])

            if tabs == 'Metadata by dashboard':
                # Folder creation 180724
                # dashb_metadata_folder = os.path.join(ROOT_PATH, project_folder,'dashboard_metadata')
                # os.makedirs(dashb_metadata_folder, exist_ok=True)
                # Folder creation end
                        ...
                        if selected_workbook != "Choose an option":
                            ...
                            if selected_dashboard != "Choose an option":
                                ...
                                if st.button('Get metadata'):
                                    ...
                                    # lineage_path = os.path.join(project_path,f'{portalname}_{selected_workbook}-{selected_dashboard}_metadata.csv')
                                    lineage_path = os.path.join(dashb_metadata_folder,f'{portalname}_{selected_workbook}-{selected_dashboard}_metadata.csv')
                                    ...
                                    if os.path.exists(lineage_path):
                                        ...
                                        metadata_df = get_lineage_df
                                        metadata_df = metadata_df[['Site_name','Project_name','Workbook_name','Dashboard_name','Datasource_name','Database_name','Database_connection','Column_name','Table_name']]
                                        st.subheader(f'Displaying the metadata for the "{selected_dashboard}" dashboard from the workbook "{selected_workbook}"')
                                        
                                        metadata_df_rows = []
                                        for i, row in metadata_df.iterrows():
                                            columns = row['Column_name'].split(',')
                                            tables = row['Table_name'].split(',')
                                            for col, table in zip(columns, tables):
                                                # Remove quotes from Table_name
                                                table = table.replace("'", "")
                                                metadata_df_rows.append([
                                                    row['Site_name'], row['Project_name'], row['Workbook_name'], row['Dashboard_name'],
                                                    row['Datasource_name'], row['Database_name'], row['Database_connection'], col.strip(), table.strip()
                                                ])
                                        normalized_df = pd.DataFrame(metadata_df_rows, columns=metadata_df.columns)
                                        normalized_df = normalized_df.sort_values(by='Datasource_name')
                                        metadata_df = normalized_df.drop_duplicates()
                    ...
                    if selected_project != "Choose an option":
                        ...
                        if selected_workbook != "Choose an option":
                            ...
                            if st.button('Get metadata'):
                                ...
                                workbook_lineage_path = os.path.join(wb_metadata_folder,f'{portalname}_{selected_workbook}_metadata.csv')
                                # workbook_lineage_path = os.path.join(project_path,f'{portalname}_{selected_workbook}_metadata.csv')
                                if os.path.exists(workbook_lineage_path):
                                    st.write(f'Metadata for the {portalname} exists')
                                    get_wb_lineage_df = pd.read_csv(workbook_lineage_path)
                                    # st.write(get_wb_lineage_df)
                                    # st.write('Writing the get_lineage_df into metadata_df')
                                    metadata_df = get_wb_lineage_df
                                    metadata_df = metadata_df[['Site_name','Project_name','Workbook_name','Dashboard_name','Datasource_name','Database_name','Database_connection','Column_name','Table_name']]
                                    st.subheader(f'Displaying the metadata for the workbook "{selected_workbook}"')
                                    metadata_df_rows = []
                                    for i, row in metadata_df.iterrows():
                                        columns = row['Column_name'].split(',')
                                        tables = row['Table_name'].split(',')
            ...
            elif tabs == 'All workbooks metadata':
                if st.button('Get metadata'):
                    conn, _ = connect_to_ts(st.session_state.config, st.session_state.env)
                    site_lineage_path = os.path.join(all_wb_metadata_folder,f'{portalname}_metadata.csv')
                    # lineage_dash_path = os.path.join(project_path,f'{portalname}_metadata.csv')
                    if os.path.exists(site_lineage_path):
                        st.write(f'Metadata for the {portalname} exists')
                        portal_metadatadf = pd.read_csv(site_lineage_path)
                        metadata_df = portal_metadatadf
                        metadata_df = metadata_df[['Site_name','Project_name','Workbook_name','Dashboard_name','Datasource_name','Database_name','Database_connection','Column_name','Table_name']]
                        st.subheader(f'Displaying metadata for all the workbooks in the site "{portalname}"')
                        metadata_df_rows = []
                        for i, row in metadata_df.iterrows():
                            columns = row['Column_name'].split(',') if pd.notna(row['Column_name']) else ['NaN']
                            tables = row['Table_name'].split(',') if pd.notna(row['Table_name']) else ['NaN']
                            for col, table in zip(columns, tables):
                                # Remove quotes from Table_name
                                table = table.replace("'", "")
                                metadata_df_rows.append([
                        ...
                        if st.button('Get portal metadata'):
                            ...
                            get_dash_lineage_df.to_csv(site_lineage_path)
                            st.write(f'Metadata for the {portalname} is generated')
                            get_prompt(site_lineage_path)
            ...
            if os.path.exists(masterfile) and os.path.exists(linkfile) and os.path.exists(tableschemafile):
                st.write(f'Required files for template matching exists')
            else:
                st.write(f'Preparing the files for template matching')
                metadatafile = os.path.join(all_wb_metadata_folder, f"{portalname}_metadata.csv")
                generate_csvs(metadatafile,portalname,template_folder)  
                st.write(f'Required files for template matching exists')
            
            # generate_csvs(metadatafile,masterfile,linkfile,tableschemafile)
            dashimg_path = os.path.join(project_path,'dashboard image')
            cache_file = os.path.join(template_folder,f'{portalname}_templatematching_cache.csv')
                        ...
                        try:
                            ...
                            if not matching_dashbd.empty:
                                ...
                                if os.path.exists(dash_img_file):
                                    ...
                                    dashboardlink = get_dashboard_link(portalname,server_endpoint_file,dashboard_id,dashboard_df)
                        ...
                        if selected_workbook != "Choose an option":
                            ...
                            if selected_dashboard != "Choose an option":
                                ...
                                # Fetch and display dashboard image
                                    ...
                                    os.makedirs(selecteddashboard_folder,exist_ok=True)
                                    lineage_path = os.path.join(dashb_metadata_folder,f'{portalname}_{selected_workbook}-{selected_dashboard}_metadata.csv')
                                    lineage_dash_path = os.path.join(all_wb_metadata_folder,f'{portalname}_metadata.csv')
                                    report_path = os.path.join(narration_folder,f'{selected_workbook}-{selected_dashboard}_narration.pdf')
                                    ...
                                    if os.path.exists(report_path):
                                        ...
                                        display_pdf(narration_folder,selected_workbook,selected_dashboard)                   
                                    else:
                                        st.write('Report creation in progress')
                                        create_pdf(cg_logo_path,selected_workbook,selected_dashboard,lineage_path,selecteddashboard_folder,dashboardimgfile,narration_folder)  
                                        st.write(f'Report created')
                                        display_pdf(narration_folder,selected_workbook,selected_dashboard)                   
